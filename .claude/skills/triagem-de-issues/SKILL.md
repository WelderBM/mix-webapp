...
- **Fundir #74 → #75** (mesma toolbar do `ProductsTab`; redesign já corrige o overflow). Caso-modelo do destino 2. **Executado em 28/07/2026** — a decisão ficou registrada aqui por dias sem virar ação; #74 fechada só nesta segunda varredura. Padrão a reconhecer: uma decisão de fusão anotada na skill não é auto-executável — se ninguém volta pra fechar a issue, ela continua viva no backlog dando a falsa sensação de trabalho pendente que já foi "decidido". A varredura periódica existe também pra isso: checar decisões antigas que nunca saíram do papel, não só achar clusters novos.
...
- **#57 ∩ #71**: fechar a parte "vitrine automática por categoria" da #57 como dup do modo `category`/`auto` da #71; recortar só a *gestão* (excluir/renomear/ocultar/drag/persistência) como sub-issue. #71 assume a arquitetura. **Executado em 27/07/2026** (corpo da #57 já reduzido).
...
- **Standalone confirmadas**: #73 (OrdersTab, superfície própria), #72 (CartSidebar). Sequenciamento #56 depois de #72 (mesma seção "Forma de pagamento" do `CartSidebar`) só existia na memória desta skill até 28/07/2026 — a dependência nunca tinha sido escrita no corpo da própria issue #56. Padrão a reconhecer: uma nota de sequenciamento só de decisão em memória de skill não protege nada se a issue for implementada por outra sessão/máquina que nunca leu a skill — a dependência real tem que estar no corpo da issue (`depende de #N`), a skill é o índice, não a fonte.
- **Dependência satisfeita vira referência morta se ninguém volta pra remover**: #82 citava "sequenciar depois da #81" — #81 já estava fechada (schema Zod de env, `env.ts` desde `889d113`) havia dias, mas a issue #82 continuava lendo como se ainda tivesse um bloqueio. Ao fechar/mergear uma issue que outras citam como dependência, considere: alguém precisa ir nas issues que a citavam remover o bloqueio? (não dá pra automatizar isso com confiança — é outro motivo pra varredura periódica.)
