---
name: unit-test
description: Use quando precisar gerar testes unitários pra código sem cobertura (happy path, edge case, erro, boundary) ou aumentar cobertura de um arquivo/módulo específico — detecta Vitest automaticamente e adota as convenções de teste já usadas no repo. NÃO use pra corrigir um bug de aplicação encontrado no processo — bug de app vira achado reportado, nunca correção automática (ver "Bug de teste vs. bug de aplicação" abaixo). NÃO substitui a `auditoria-de-pr` pra revisar a lógica de um PR já pronto.
---
...
- Removido o framing "AUTONOMOUS MODE. Do NOT ask questions" — a skill gera e roda testes, mas o commit/PR final segue o fluxo fatiado normal do projeto (ritual `tsc`/`vitest`/`build` + `auditoria-de-pr` antes de qualquer PR).

---

You detect the project's test framework, find untested code, generate comprehensive unit tests, run them, and self-heal *test* failures in a loop. You do not modify application code — an application bug found along the way is a reported finding, not a silent fix.
...
Neste repo o framework é sempre Vitest (`vitest.config.ts` na raiz) — a detecção acima é herdada do skill original pra manter a skill reutilizável em outros projetos, mas aqui você já sabe a resposta.
...
- Database queries and connections (Firestore incluso)
...
============================================================
PHASE 4: EXECUTION AND SELF-HEALING (SOMENTE BUG DE TESTE)
============================================================
...
Step 4.2 -- Bug de teste vs. bug de aplicação

For each failing test, diagnose the failure:

CATEGORIA A — BUG DE TESTE (corrija o teste, autônomo, até 3 iterações):
...
CATEGORIA B — BUG DE APLICAÇÃO (**NÃO corrija — reporte**):
...
Se o diagnóstico apontar categoria B, **pare** de tentar fazer aquele teste passar. Registre o achado (arquivo, função, input que quebra, comportamento esperado vs. observado) na saída final, sob "Bugs de aplicação encontrados (não corrigidos)". A decisão de como corrigir — inclusive se o valor ausente precisa de um guard explícito em vez de um fallback — é de quem revisar o achado, não da geração de teste.

ITERATION RULES (categoria A apenas):
...
Se testes existentes quebraram, isso também é um achado a reportar — não corrija código de aplicação pra "consertar" silenciosamente.
...
### Self-Healing Summary (bug de teste)
- Iteration 1: N fixed
- Iteration 2: N fixed
- Iteration 3: N fixed
- Unresolved: N (with reasons)

### Bugs de aplicação encontrados (não corrigidos)

| Arquivo | Função | Input que quebra | Esperado | Observado |
|---|---|---|---|---|
...
- Do NOT weaken assertions to make tests pass. Fix the test logic — never the application code.
- Do NOT edit application/production code, ever, for any reason, under this skill.
