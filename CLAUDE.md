...
- **Nenhum fallback (`a || b`) ou comparação de igualdade envolvendo um campo opcional pode ficar sem guard explícito** verificando se o campo está de fato preenchido antes de usar o valor. Causa raiz de dois incidentes reais: #52 (`rollPrice || price` deixava um `rollPrice` não configurado cair silenciosamente pro preço por metro, cobrando R$0,40 no lugar de R$40) e #53 (dedup de carrinho comparando `kitName === kitName`, onde `undefined === undefined` sempre batia e colapsava itens distintos). Os dois têm a mesma forma: valor errado mas plausível, sem exceção lançada — passa no `tsc`, passa em teste que não cobre o caso, passa em review superficial. Antes de escrever `a || b` ou `x === y` com um campo `?:`, pergunte "o que acontece se o campo não estiver preenchido dos dois lados?"; se a resposta for "algo plausível mas errado", troque por um guard explícito (`!= null`, checagem de estado, type guard dedicado) — ver `src/lib/ribbon-pricing.ts` (`hasMeterPrice`/`hasRollPrice`) pro padrão a seguir.
