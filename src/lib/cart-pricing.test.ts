...
import { getCartItemTotal, getCartItemUnitPrice } from "./cart-pricing";
...
describe("getCartItemTotal", () => {
  it("charges an open ribbon by meter, not by roll price (production incident regression)", () => {
    // The actual incident: 99m of an open ribbon billed as 99 x rollPrice
    // (R$40) = R$3.960 instead of 99 x price-per-meter.
    const item: CartItem = {
      cartId: "c1",
      type: "SIMPLE",
      quantity: 99,
  ...
  it("returns 0 for a SIMPLE item with no product and no variant", () => {
